try:
    # Python 3
    from urllib.request import BaseHandler, HTTPPasswordMgrWithDefaultRealm, build_opener, ProxyHandler, HTTPBasicAuthHandler, HTTPRedirectHandler
except ImportError:
    # Python 2
    from urllib2 import BaseHandler, HTTPPasswordMgrWithDefaultRealm, build_opener, ProxyHandler, HTTPBasicAuthHandler, HTTPRedirectHandler
...
def get_proxy_handler(proxyhost, proxyport, proxyuser, proxypass):
    ...
    proxy_handler = ProxyHandler( proxy_spec )
...
class NoAuto302Handler(HTTPRedirectHandler):
